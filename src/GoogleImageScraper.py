...
class GoogleImageScraper:
    def __init__(self, category_dir: str, class_name: str, worker_id: int):
        ...
        if is_cache_complete(self.category_dir, class_name):
            logger.info(f"Skipping scraper for '{self.query}' — already completed")
            self.skip = True
            return
        self.skip = False
        ...
        self.url_fetcher = UrlFetcher(self.category_dir, class_name, self.worker_id)
        self.image_downloader = ImageDownloader(self.category_dir, class_name, self.worker_id)
        logger.info(f"Initialized scraper for '{self.query}'")

    def fetch_image_urls(self):
        if self.skip:
            logger.info(f"Skipping URL fetch — already done for '{self.query}'")
            return []

        logger.status(f"Searching for '{self.query}' images")
        try:
            image_urls = self.url_fetcher.find_image_urls()
            if image_urls:
                logger.success(f"Found {len(image_urls)} images for '{self.query}'")
            else:
                logger.warning(f"No images found for '{self.query}'")
            return image_urls
        except Exception as e:
            logger.error(f"Failed to fetch URLs for '{self.query}': {e}")
            return []

    def download_images(self, image_urls):
        if self.skip:
            logger.info(f"Skipping download — already done for '{self.query}'")
            return 0
        ...
        logger.status(f"Downloading {len(image_urls)} images for '{self.query}'")
        saved_count = self.image_downloader.save_images(
            image_urls=image_urls,
            keep_filenames=cfg.KEEP_FILENAMES
        )
        if saved_count > 0:
            logger.success(f"Downloaded {saved_count} new images for '{self.query}'")
        else:
            logger.warning(f"No new images downloaded for '{self.query}'")
        return saved_count

    def close(self):
        if self.skip:
            return
        logger.info(f"Cleaning up resources for '{self.query}'")
